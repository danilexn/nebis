...
class PoolingPMA(BasePooler):
    def __init__(self, config):
        ...
        self.S = nn.Parameter(torch.Tensor(1, self.config.k_seeds, self.config.embedding_size))

        self.rFF_S_mutome = nn.Linear(self.config.embedding_size, self.config.embedding_size, bias=False)
...
class PoolingISAB(BasePooler):
    def __init__(self, config):
        ...
        self.S = nn.Parameter(torch.Tensor(1, self.config.k_seeds, self.config.embedding_size))
        self.I = nn.Parameter(torch.Tensor(1, self.config.k_identity, self.config.embedding_size))

        self.rFF_S_mutome = nn.Linear(self.config.embedding_size, self.config.embedding_size, bias=False)
        self.rFF_I_mutome = nn.Linear(self.config.embedding_size, self.config.embedding_size, bias=False)
...
_pooling_dict = {"max": PoolingMax, "min": PoolingMin, "mean": PoolingMean,
                 "PMA": PoolingPMA, "ISAB": PoolingISAB, "MAB": PoolingMAB,
                 "RNN_sum": PoolingRNNsum, "RNN_mean": PoolingRNNmean, "RNN_PMA": PoolingRNNPMA}
